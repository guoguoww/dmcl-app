        ...
        <!-- <section class="clearfix"> -->
          ...
          <div class="right-buy">
            ...
            <div class="open-time c-div" v-if="simulate==1">
              <span style="margin-right: 52px;">策&nbsp;略&nbsp;匹&nbsp;配</span>
              <div class="delay_tip">
                <ul
                  id="buy_price_ul"
            ...
            <!--  -->
            <button id="btn_buy" class="color-btn " :class="in_time_range||userInfo.tradeSimulate!=1?'':'disabled'" @click="order">{{$route.params.simulate==2?'体验':''}}点买</button>
...
export default {
  ...
  computed: {
    ...
    lossRange(){
     ...
     if (money&&minBase) {
       if (!this.loss) {
        this.loss=(money-(((this.selectMoney*maxBase)/(this.numberOfShares*money))*money)).toFixed(2)
       }
     }
  ...
  methods: {
    ...
    order(){
      ...
      if (this.simulate==2) {
        ...
        this.surplus=((+this.market[3])*(1+(+this.fees.minProfit))).toFixed(2)     
        this.loss=((+this.market[3])*(1-(+this.fees.minLoss))).toFixed(2)
      }
      this.$axios({
